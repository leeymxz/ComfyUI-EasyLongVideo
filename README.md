...
- 每段卡片：试听、查看识别文本、编辑镜头简报（自动保存）、
  「并入下一段」「✂ 拆分」（在候选切点处一分为二）、「重做本段」；
- 顶部整曲播放器 + 「▶ 连播全部分段」按顺序试听；
- 左上角**历史项目下拉**可切换打开任何旧项目（继续重做/重新合成）；
- 底部可设置**失败自动重试**次数（0=不重试）；
- 底部「保存并确认」后即可生成。

**节点上的实时进度**：顺序生成时节点会显示「正在生成第 x/N 段」、
自动重试与完成状态，无需打开面板盯守。
